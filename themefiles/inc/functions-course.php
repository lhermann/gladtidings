...
function add_remove_lesson_columns( $columns ) {
	$columns = array(
		...
		"type" 				=> '',
		"title" 			=> __( 'Title' ),
		"order" 			=> '<span title="'.__( 'Order', 'gladtidings' ).'" class="dashicons dashicons-editor-ol"></span>',
	...
	switch ( $column_name ) {
		case 'order':
			if ( $term->parent == 0 ) {
				...
				print( get_field( 'course-order', 'course_'.$id ) );
			}
			break;
	}
}
...
function lesson_courses_ordering( $clauses, $wp_query ) {
	...
	//var_dump( $wp_query->query, $clauses );
	if ( isset( $wp_query->query['orderby'] ) && $wp_query->query['orderby'] == 'menu_order title' ) {
		$clauses['orderby'] = false;
	} elseif ( isset( $wp_query->query['orderby'] ) && $wp_query->query['orderby'] == 'order' ) {
...
function test123( $v1, $v2, $v3 ) {
	var_dump( $v1, $v2, $v3 );
	//var_dump( $v1, $v2, $v3 ); die();
}
//add_action( "posts_clauses", "test123", 10, 3 );
